...
from app.handlers import adminRouter, userRouter
...
async def main():
    ...
    scheduler.add_job(rem.brigade_report, trigger='cron', hour=10, minute=0, start_date=datetime.now(), kwargs={'bot': bot})
    scheduler.add_job(rem.table_update, trigger='cron', day=4, hour=10, minute=0, start_date=datetime.now(), kwargs={'bot': bot})
    scheduler.start()
